                ...
                <div class="flex items-center justify-between d p-4">
                    ...
                    <div class="flex space-x-3">
                        <div class="flex space-x-3 items-center">
                            <label class="w-40 text-sm font-medium text-gray-900 dark:text-white">Validation
                                Status:</label>
                        ...
                        <tbody>
                            @foreach ($iPlanRecords as $iPlanRecord)
                                <tr class="dark:bg-gray-900 dark:border-gray-700 text-white bg-gray-500">
                                    ...
                                    <td class="px-4 py-3 dark:text-white">
                                        {{ $iPlanRecord->iPLAN === 'OK' ? 'Cleared' : 'Not Cleared' }}</td>
                                    <td class="px-4 py-3 flex items-center gap-3">
                                        <div class="relative group inline-block">
                                            <a href="{{ $userType === 'IBUILD'
                                                ? route('ibuild.view-subproject', $iPlanRecord->id)
                                                : ($userType === 'IPLAN'
                                                    ? route('iplan.view-subproject', $iPlanRecord->id)
                                                    : ($userType === 'ECON'
                                                        ? route('econ.view-subproject', $iPlanRecord->id)
                                                        : ($userType === 'SES'
                                                            ? route('ses.view-subproject', $iPlanRecord->id)

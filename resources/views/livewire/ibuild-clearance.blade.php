                        ...
                        <tbody>
                            @foreach ($iBuildRecords as $iBuildRecord)
                                <tr class="dark:bg-gray-900 dark:border-gray-700 text-white bg-green-700">
                                    ...
                                    <td class="px-4 py-3 dark:text-white">
                                        {{ $iBuildRecord->iBUILD === 'OK' ? 'Cleared' : 'Not Cleared' }}</td>
                                    <td class="px-4 py-3 flex items-center gap-3">
                                        <div class="relative group inline-block">
                                            <a href="{{ $userType === 'IBUILD'
                                                ? route('ibuild.view-subproject', $iBuildRecord->id)
                                                : ($userType === 'IPLAN'
                                                    ? route('iplan.view-subproject', $iBuildRecord->id)
                                                    : ($userType === 'ECON'
                                                        ? route('econ.view-subproject', $iBuildRecord->id)
                                                        : ($userType === 'SES'
                                                            ? route('ses.view-subproject', $iBuildRecord->id)

<div>
    ...
    <div class="overflow-x-auto rounded-md">
        <table class="w-full text-sm text-left">
            ...
            <tbody>
                @foreach ($iReapRecords as $iReapRecord)
                <tr class="dark:bg-gray-900 dark:border-gray-700 text-white bg-green-700">
                    <th scope="row" class="px-4 py-3">
                        {{ $iReapRecord->projectName }}
                    </th>
                    <td class="px-4 py-3 dark:text-white">{{ $iReapRecord->proponent }}</td>
                    <td class="px-4 py-3 dark:text-white">
                        {{ $iReapRecord->indicativeCost }}
                    </td>
                    <td class="px-4 py-3 dark:text-white">
                        {{ $iReapRecord->iREAP === 'OK' ? 'Cleared' : 'Not Cleared' }}
                    </td>
                    <td class="px-4 py-3 flex items-center gap-3">
                        <div class="relative group inline-block">
                            <?php
                            $route = match ($userType) {
                                'IBUILD' => route('ibuild.view-subproject', $iReapRecord->id),
                                'IPLAN' => route('iplan.view-subproject', $iReapRecord->id),
                                'ECON' => route('econ.view-subproject', $iReapRecord->id),
                                'SES' => route('ses.view-subproject', $iReapRecord->id),
                                'GGU' => route('ggu.view-subproject', $iReapRecord->id),
                                'IREAP' => route('ireap.view-subproject', $iReapRecord->id),
                                default => '#',
                            };
                            ?>
